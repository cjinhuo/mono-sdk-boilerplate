...
默认执行 publish、创建 Git tag 并推送 tag。`--no-git-tag` 会透传给 Changesets，并跳过 tag 推送。CI 通过 `changesets/action/publish@v2` 调用本封装，使 tag 指向刚生成的 version commit，同时由 Action 创建 GitHub Release；本封装也继续用于本地或手动直发。
